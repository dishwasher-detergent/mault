...
export const data = new SlashCommandBuilder()
  ...
  .addStringOption((opt) =>
    opt
      .setName("collection")
      .setDescription(
        "Only route this collection's scans here (defaults to every collection)",
      )
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  await interaction.editReply(
    collectionGuid
      ? `Card scans for that collection will now be posted in <#${channel.id}>.`
      : `Card scans will now be posted in <#${channel.id}>.`,
  );
}
