import type { SearchCardResponse } from "@/interfaces/api.interface";
import type {
	CardScannerProps,
	DetectionResult,
	ScannerStatus,
...
export function useCardScanner({
				...
				if (captureCanvas && captureResult?.detected && captureResult.contour) {
					...
					canvasToBlob(cardCanvas)
						...
						.then(async (response) => {
							...
							return response.json() as Promise<SearchCardResponse>;
						})
						.then(async (data) => {
							if (data.match) {
								const scryfallCard = await fetchCardById(data.match.scryfallId);
								...
								if (lastScannedCardIdRef.current === scryfallCard.id) {
									// Duplicate — show prompt instead of auto-adding
	...
	const handleForceScan = useCallback(() => {
		...
		// Use detected contour if available, otherwise send the full frame
		const cardCanvas =
