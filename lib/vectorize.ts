import { AutoProcessor, RawImage, SiglipVisionModel, type Processor } from "@huggingface/transformers";
...
async function getModel(): Promise<SiglipVisionModel> {
	if (!modelPromise) {
		...
		await modelPromise;
		console.log("[vectorize] SigLIP model loaded successfully (768 dimensions)");
	}
	return modelPromise;
}
